...
- 명령창에서 아래가 동작해야 합니다:
  ...
  ```
  회사 환경에 따라 명령 이름이 `abaqus` / `abq2025` / `abq2025hf4` 중
  하나일 수 있습니다. 동작하는 이름을 확인해 두세요.
...
실행 드라이버 배치파일에 주입**되어 있습니다. 예:
```
C:\SIMULIA\Commands\abq2025hf4.bat  <-- 이 안에 컴파일러 환경 로드가 삽입됨
```
그래서 **일반 셸에서 `ifort` 단독 호출은 안 보이는 게 정상**이고,
`abaqus`(=이 드라이버)를 통해 부를 때만 컴파일이 됩니다.
> 재설치 시 이 주입이 사라지므로, `abq2025hf4.bat.orig` 같은 백업을 두고
> 다시 적용하는 관리가 필요합니다.
