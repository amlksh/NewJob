...
- **역할 경계**: 임플란트 응력·변형, 연조직 변형, 상세 접촉, 피로는 **SIMULIA 담당**이며 OpenSim 에서 개발하지 않는다.
  OpenSim 담당은 근골격 동역학 — 관절가동범위, 관절 모멘트, 근육력, 관절반력이며, 이것이 SIMULIA 해석의 경계조건이 된다.
  이 경계가 Reference 모델 선정 근거다 (ADR-0002). 인대력·연골 접촉압을 OpenSim 에서 내려는 구성은 경계를 넘는다.
...
- 기기 파라미터를 바꾸면 `source.revision` 을 갱신한다. `TBD` 인 기기로 낸 결과는 `citable: false` 로 기록되며 대외 인용·납품에 쓰지 않는다.
- 파이프라인을 바꿔 토이 회귀 기준선(V-01)이 움직이면, `scripts/refresh_toy_baseline.py` 로 갱신하고 **그 diff 를 커밋에 남겨** 검토받는다. 기준선을 조용히 갱신하지 않는다.
- 결과를 내보낼 때 **품질 지표를 함께 내보낸다.** SIMULIA 측은 받은 하중의 신뢰도를 알 방법이 그것뿐이다.
- 설정을 바꿀 때마다 이유를 남긴다. 마커 가중치·필터 차단주파수는 결과를 직접 바꾸므로 누가 언제 왜 바꿨는지가 V&V 기록이 된다.
- 검증 기록은 다음 체인을 남긴다:
  `Input → Model Version → Parameter → Execution → Output → Reference Data → Acceptance Criteria → Pass/Fail`

### 도구의 성공 반환을 믿지 않는다 — 실제 사례

OpenSim 도구는 **실패해도 성공을 반환하는 경우가 있다.** 파일 존재만 확인하면
"해석 완료"로 보이면서 값이 없거나 틀린다. 아래는 이 저장소에서 실측으로 확인한
사례이며, 새 단계를 붙일 때 같은 방식으로 출력을 직접 검사한다.

| 사례 | 겉보기 | 실제 | 방어 |
| --- | --- | --- | --- |
| **JointReaction 의 `joint_names` 가 모델의 관절 이름과 다름** | `run()` 이 `True`, 결과 파일도 생성됨 | `time` 열만 있는 **빈 결과**. 관절반력이 통째로 없다 | `steps.check_reaction_output()` 이 반력 열 존재를 확인하고 없으면 `StepExecutionError` |
| Setup XML 의 경로가 상대경로 | `run()` 이 `True` | OpenSim 이 Setup XML 디렉터리로 작업 디렉터리를 옮겨, 출력이 없는 경로를 가리키고 아무것도 쓰이지 않음 | `run_dir` 를 절대경로로 고정 |
| 해석 구간이 `0 0` 으로 남음 | 정상 종료 | IK 가 **한 프레임만** 풂 | 구간을 Case·마커 파일에서 치환, 못 정하면 중단 |
| `MarkerPlacer` 의 `IKTaskSet` 이 비어 있음 | — | 예외가 아니라 **segmentation fault**. 프로세스가 사라져 `provenance.json` 도 안 남는다 | `steps.check_marker_placer_tasks()` 가 호출 전에 차단 |
...
- **B-4(합격기준 확정) 전에 B-5(검증 trial 재현)를 착수하지 않는다.** 검증 결과를 보고 기준을 정하면 검증이 아니다.
- 달성된 결과에 맞춰 합격기준을 사후 완화하지 않는다. 못 맞추면 그 사실을 결과서에 적는다.
- **토이 모델 테스트를 삭제하거나 완화하지 않는다.** 실데이터 허용치와 하나로 합치지 않는다 — 둘은 다른 것을 잡는다.
- **Device OFF/ON 차이를 정량 수치로 인용하지 않는다.** 비교기준 문헌 3건은 등재되었으나(`docs/vv/traceability.md` §기기 결합 검증 비교기준) **CAD 수치가 A-1 회신 전에는 예시값**이다. 상대 경향으로만 표기한다.
- **OFF/ON 델타를 보행 주기 평균으로만 보고하지 않는다.** 기기 질량은 초기 입각 하중을 올리고 보조 토크는 후기 입각 모멘트를 내리므로, 평균을 내면 상쇄되어 "영향 없음" 으로 보인다 (McGibbon 2017). **초기·중기·후기 입각으로 분리**해 보고한다.
- OpenSim 버전을 `4.x` 로 느슨하게 쓰지 않는다. **4.6 고정**이며 버전 변경은 V&V 재수행 대상이다 (ADR-0001).

### 검증 방법

- 코드 변경 후: `pytest -q` (OpenSim 미설치 환경에서는 `pytest -q -m "not opensim"`)
- 토이 회귀 기준선이 어긋나면 **파이프라인이 바뀐 것이다.** 실데이터 수치가 나빠졌을 때 원인을 가르는 기준이므로, 먼저 여기를 확인한다.
